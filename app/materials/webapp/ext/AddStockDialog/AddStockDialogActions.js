sap.ui.define([
  ...
  async function getDialog() {
    if (!_oDialog) {
      ...
      // диалогу нужна модель, чтобы отработал items="{path: '/Warehouses'}" у Select
  ...
  async function onConfirmAddStock(oEvent) {
    ...
    const oOperation = oModel.bindContext("/addStock(...)"); // unbound action — без контекста сущности
    ...
    try {
      ...
      _oMaterialContext.getBinding().refresh(); // обновит "Stock by Warehouse" на Object Page
